...
namespace SmartAdmin.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        private readonly IServices<UserSetting> _userSettingService;
        ...
        public AccountController(
            IServices<Pipeline> pipelineService,
            IServices<Stage> stageService,
            IServices<TaskGroup> taskGroupService,
            ...
            IServices<UserBusinessEntity> userBusinessEntityService,
            IServices<UserSetting> userSettingService,
            IHttpContextAccessor accessor)
        {
            ...
            this._userBusinessEntityService = userBusinessEntityService;
                ...
                if (user != null)
                {
                    if (!await _userManager.IsEmailConfirmedAsync(user))
                    {
                        ModelState.AddModelError(string.Empty,
                                      "Você deve confirmar seu e-mail para efetuar login.");
                        return View(model);
                    }
                }
        ...
        public async Task<IActionResult> Register(RegisterViewModel model, byte[] AvatarImage = null, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    var userSetting = new UserSetting("", model.Name, "", "", businessEntity, user) { AvatarImage = AvatarImage, UserSettingId = user.Id };
                    ...
                    if (_pipelineService.Count() <= 0)
                    {
                        ...
                        var pipeline = new Pipeline("Pipeline") { BusinessEntity = businessEntity, UserSettingId = user.Id };
                        List<Stage> stage = new List<Stage>();
                        List<TaskGroup> taskGroup = new List<TaskGroup>();


                        stage.Add(new Stage() { BusinessEntity = businessEntity, Pipeline = pipeline, OrderStage = 1, Name = "Cliente potencial" });
                        stage.Add(new Stage() { BusinessEntity = businessEntity, Pipeline = pipeline, OrderStage = 2, Name = "Contatado" });
                        stage.Add(new Stage() { BusinessEntity = businessEntity, Pipeline = pipeline, OrderStage = 3, Name = "Demo Agendada" });
                        stage.Add(new Stage() { BusinessEntity = businessEntity, Pipeline = pipeline, OrderStage = 4, Name = "Proposta Feita" });
                        stage.Add(new Stage() { BusinessEntity = businessEntity, Pipeline = pipeline, OrderStage = 5, Name = "Negociações começadas" });

                        taskGroup.Add(new TaskGroup() { BusinessEntity = businessEntity, Name = "Ligação" });
                        taskGroup.Add(new TaskGroup() { BusinessEntity = businessEntity, Name = "Reunião" });
                        taskGroup.Add(new TaskGroup() { BusinessEntity = businessEntity, Name = "Email" });
                        taskGroup.Add(new TaskGroup() { BusinessEntity = businessEntity, Name = "Almoço" });
                        taskGroup.Add(new TaskGroup() { BusinessEntity = businessEntity, Name = "Tarefa" });
                        taskGroup.Add(new TaskGroup() { BusinessEntity = businessEntity, Name = "Apresentação" });
        ...
        public async Task<IActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                await _emailSender.SendEmailAsync(model.Email, "Password Reset",
                   $"<p>Você solicitou a <b> recuperação da sua senha </b>, por favor clique no link <a href='{callbackUrl}'>link</a>"  +"</p>" +
                   "<p>" + "Caso não reconheça esta atividade, recomendamos fortemente a troca da sua senha." +"</p>"
                   );
